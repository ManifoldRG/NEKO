...
def get_linear_warmup_cosine_decay_scheduler(optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, base_lr: float, init_lr: float, min_lr: float , last_epoch=-1):
...
def _linear_warmup_cosine_decay(current_step: int, *, num_warmup_steps: int,num_training_steps: int, base_lr: float, init_lr: float, min_lr: float):

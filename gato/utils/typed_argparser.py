...
from typing import Any, Callable, Dict, Iterable, List, Literal, NewType, Optional, Tuple, Union, get_type_hints, TypeVar, Generic
...
def TypedArg(
    *,
    ...
    default_factory: Union[Callable[[], Any], dataclasses._MISSING_TYPE] = dataclasses.MISSING,
    metadata: Optional[dict] = None,
    **kwargs,
) -> dataclasses.Field:
    """Argument helper enabling a concise syntax to create dataclass fields for parsing with `TypedArgumentParser`.

    Example comparing the use of `TypedArg` and `dataclasses.field`:
    ```
    @dataclass
    class Args:
        regular_arg: str = dataclasses.field(default="Huggingface", metadata={"aliases": ["--example", "-e"], "help": "This syntax could be better!"})
        typed_arg: str = TypedArg(default="Huggingface", aliases=["--example", "-e"], help="What a nice syntax!")
    ```
...
class TypedArgumentParser(ArgumentParser, Generic[DataClass, DataClassType]):
    ...
    def parse_args_into_dataclasses(
        ...
        if return_remaining_strings:
            ...
            if remaining_args:
                raise ValueError(f"Some specified arguments are not used by the TypedArgumentParser: {remaining_args}")
    ...
    def parse_dict(self, args: Dict[str, Any], allow_extra_keys: bool = False) -> Tuple[DataClass, ...]:
        ...
        if not allow_extra_keys and unused_keys:
            raise ValueError(f"Some keys are not used by the TypedArgumentParser: {sorted(unused_keys)}")

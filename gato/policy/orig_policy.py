...
class GatoPolicy(nn.Module):
    ...
    def forward(self, inputs: list = None, compute_loss=False, **kwargs):
        ...
        output = self.transformer(inputs_embeds=token_embeddings, attention_mask=token_masks, use_cache=True)
        final_representations = output['last_hidden_state']
        last_key_values = output['past_key_values']
        ...
        if compute_loss:
            ...
            loss = None
        return logits, loss, last_key_values
    ...
    def predict_control(self, input: dict, task: ControlTask, current_timestep: int, deterministic: bool = True, past_key_values = None):
        ...
        # remove constant padding tokens at start of seq
        ...
        for i in range(action_tokens):
            logits, _, past_key_values = self.forward(token_embeddings=token_embeddings, token_masks=token_masks, token_target_masks=None, tokens=None)
            print(current_timestep, past_key_values[0].sum())
            import pdb; pdb.set_trace()
